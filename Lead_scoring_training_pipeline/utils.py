...
def encode_features():
    ...
    **NOTE : You can modify the encode_featues function used in heart disease's inference
    ...
    try:
        cnx = sqlite3.connect(DB_PATH+DB_FILE_NAME)
        if not check_if_table_has_value(cnx,'target') or not check_if_table_has_value(cnx,'features'):
...
def get_trained_model():
    ...
    try:
        ...
        if check_if_table_has_value(cnx,'features') and check_if_table_has_value(cnx,'target'):
            ...
            print("Features or Target does not  exists")
    except Exception as e:
        print (f'Exception thrown in get_trained_model : {e}')
    finally:
        if cnx:        
            cnx.close()
